...
class shunqing(scenario):
    ...
    def step(self, advance_seconds = None, log=True):
        ...
        for typ, attribute, weight in self.config["performance_targets"]:
            ...
            __lastvolume = [self.data_log[attribute][ID][-2] if len(self.data_log[attribute][ID]) > 1 else 0 for ID in
                            features]
    ...
    def get_edge_list(self):
        ...
        # 解压操作符 * 用于分离元组中的元素,从[105，105] into [2,n]
        first_elements, second_elements = zip(*edges)
        # 转换为列表形式
        first_list = list(first_elements)
        second_list = list(second_elements)
        edges = [first_list[:], second_list[:]]
